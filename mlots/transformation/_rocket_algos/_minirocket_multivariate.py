...
@njit("float64[:](float64 [:,:,:],int32[:],int32[:],int32[:],int32[:],float64[:],int32)", fastmath=True, parallel=False,
      cache=True)
def _fit_biases(X, num_channels_per_combination, channel_indices, dilations, num_features_per_dilation, quantiles,
                random_seed): # pragma: no cover
    ...
    for dilation_index in range(num_dilations):
        ...
        for kernel_index in range(num_kernels):
            ...
            C_alpha = np.zeros((num_channels_this_combination, input_length), dtype=np.float64)
            C_alpha[:] = A

            C_gamma = np.zeros((9, num_channels_this_combination, input_length), dtype=np.float64)
            C_gamma[9 // 2] = G
...
def _fit(X, num_features=10_000, max_dilations_per_kernel=32, random_seed=1992): # pragma: no cover
    ...
    biases = _fit_biases(X=X, num_channels_per_combination=num_channels_per_combination, channel_indices=channel_indices,
                         dilations=dilations, num_features_per_dilation=num_features_per_dilation, quantiles=quantiles,
...
@njit("float64[:,:](float64[:,:,:],Tuple((int32[:],int32[:],int32[:],int32[:],float64[:])))", fastmath=True,
...
def _transform(X, parameters): # pragma: no cover
    ...
    for example_index in prange(num_examples):
        ...
        for dilation_index in range(num_dilations):
            ...
            C_alpha = np.zeros((num_channels, input_length), dtype=np.float64)
            C_alpha[:] = A

            C_gamma = np.zeros((9, num_channels, input_length), dtype=np.float64)
            C_gamma[9 // 2] = G
