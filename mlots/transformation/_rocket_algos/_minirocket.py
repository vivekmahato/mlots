...
@njit("float64[:](float64[:,:],int32[:],int32[:],float64[:],int32)", fastmath=True, parallel=False, cache=True)
...
def _fit(X, num_features=10_000, max_dilations_per_kernel=32, random_seed=1992): # pragma: no cover
    ...
    biases = _fit_biases(X, dilations, num_features_per_dilation, quantiles, random_seed=random_seed)
...
@njit("float64[:,:](float64[:,:],Tuple((int32[:],int32[:],float64[:])))", fastmath=True, parallel=True, cache=True)
